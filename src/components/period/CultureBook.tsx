...
import type { CulturePageData, ContentItem } from "@/types/period-data";
...
function ContentBlock({ item }: { item: ContentItem }) {
  ...
  if (item.type === "paragraph") {
    return (
      <p style={{ fontFamily: "var(--font-sans)", fontSize: 14, lineHeight: 1.8, color: C.secondary, margin: "0 0 14px", width: "100%" }}>
        {item.text}
      </p>
    );
  }
  if (item.type === "list") {
    return (
      <ul style={{ margin: "0 0 14px", paddingLeft: 0, listStyle: "none", width: "100%" }}>
        {item.items.map((it, i) => (
          <li key={i} style={{ fontFamily: "var(--font-sans)", fontSize: 14, lineHeight: 1.75, color: C.secondary, display: "flex", gap: 10, marginBottom: 6 }}>
      ...
      <div style={{ margin: "0 0 14px", width: "100%" }}>
        {item.entries.map((e, i) => (
          <div
            key={i}
            style={{
              display: "flex", gap: 12, marginBottom: 6, alignItems: "baseline",
              padding: "7px 12px",
              background: i % 2 === 0 ? "rgba(255,255,255,0.03)" : "transparent",
              borderRadius: 4,
            }}
          >
            <span style={{ fontFamily: "var(--font-sans)", fontSize: 13, fontWeight: 700, color: C.ochre, flexShrink: 0, minWidth: 110 }}>
              {e.term}
            </span>
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center", gap: 20, padding: "40px 48px" }}>
      ...
      {onGoToQuiz && (
        <button
          onClick={onGoToQuiz}
          style={{
            marginTop: 16, padding: "13px 40px", background: C.ochre, color: "#12171F",
            border: "none", borderRadius: 6, fontSize: 15, fontFamily: "var(--font-sans)",
...
interface CultureBookProps {
  pages: CulturePageData[];
  onGoToQuiz: () => void;
}

export default function CultureBook({ pages, onGoToQuiz }: CultureBookProps) {
  ...
  function goNext() {
    if (pageIndex < totalPages - 1) { setDirection(1); setPageIndex((i) => i + 1); }
  }
  function goPrev() {
  ...
  return (
    <motion.div
      ...
      style={{ width: "100%", height: "100%", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}
    >
      {/* Üst kontroller */}
      <div style={{ position: "absolute", top: 12, right: 20, zIndex: 10, display: "flex", alignItems: "center", gap: 16 }}>
        <span style={{ fontFamily: "var(--font-sans)", fontSize: 11, color: C.secondary, letterSpacing: "0.08em" }}>
          {pageIndex + 1} / {totalPages}
        </span>
        <button
          onClick={onGoToQuiz}
          style={{ fontFamily: "var(--font-sans)", fontSize: 11, color: C.secondary, background: "transparent", border: `1px solid ${C.border}`, borderRadius: 4, padding: "5px 14px", cursor: "pointer" }}
        >
          Kültür bölümünü geç →
        </button>
      </div>
      ...
      <div style={{
        ...
        {!page.isCover && !page.isFinal && (
          <div style={{
            flexShrink: 0, height: 36, borderTop: `1px solid ${C.border}`,
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 40, marginTop: 20 }}>
        <button
          ...
          style={{
            ...
            letterSpacing: "0.06em", padding: "6px 0", transition: "color 0.2s",
          }}
        >
          ← Önceki
        </button>
        ...
        {isLast ? (
          <button
            ...
            style={{
              ...
              color: "#12171F", background: C.ochre, border: "none", borderRadius: 6,
          ...
          <button
            onClick={goNext}
            style={{
              fontFamily: "var(--font-sans)", fontSize: 13, color: C.secondary,

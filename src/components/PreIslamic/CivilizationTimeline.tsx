...
import type { Civilization, CivEvent } from "@/types";
...
const TIMELINE_START = -750;
const TIMELINE_END = 1300;
const TOTAL_SPAN = TIMELINE_END - TIMELINE_START;

function yearToPercent(year: number): number {
  return ((year - TIMELINE_START) / TOTAL_SPAN) * 100;
}

function formatYear(year: number): string {
  return year < 0 ? `MÖ ${Math.abs(year)}` : `${year}`;
}
    ...
    <div className="w-full h-full bg-[#0D1117] border-t border-white/[0.06] flex flex-col select-none">
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-1 flex-shrink-0">
        <p className="font-sans text-[10px] uppercase tracking-[0.2em] text-secondary/50">
          Kronoloji — {TIMELINE_START < 0 ? `MÖ ${Math.abs(TIMELINE_START)}` : TIMELINE_START} – MS {TIMELINE_END}
        </p>
        {selectedCivId && (
          <button
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar px-4 pb-3"
      >
        <div className="relative min-w-[900px] h-full pt-6">
          ...
          <div className="relative space-y-1 mt-0">
            {sorted.map((civ, i) => {
              const leftPct = yearToPercent(civ.startYear);
              const widthPct = yearToPercent(civ.endYear) - leftPct;
              const isSelected = civ.id === selectedCivId;
              const isDimmed = selectedCivId !== null && !isSelected;
                ...
                >
                  ...
                  <motion.div
                    ...
                    style={{
                      ...
                      backgroundColor: isSelected
                        ? civ.color
                        : `${civ.color}55`,
                      border: `1px solid ${civ.color}${isSelected ? "ff" : "44"}`,
                      opacity: isDimmed ? 0.25 : 1,
                      boxShadow: isSelected
                        ? `0 0 12px 2px ${civ.color}44`
                        : "none",
                    }}
                  ...
                  >
                    <span
                      ...
                      style={{
                        color: isSelected ? "#fff" : civ.color,
